...
`RichText` 是独立富文本元素，适合同一段文字中混合字体、字号、颜色、加粗、斜体、下划线、字距和硬换行。它不解析 HTML/Markdown。

| 方法 | 参数 | 返回 | 说明 |
| --- | --- | --- | --- |
| `.text(value)` | `str` | `RichText` | 清空 runs，并设置为一个普通文本 run |
| `.span(text, font=None, font_family=None, font_size=None, color=None, bold=False, italic=False, underline=False, letter_spacing=None)` | 文本和 span 样式 | `RichText` | 追加行内片段 |
...
rich = (
    ...
    .span("+18%", font="Helvetica", font_size=14, color="#166534", bold=True, underline=True, letter_spacing="0.08em")
    .br()
...
`italic=True` 会优先使用注册字体族的 italic / bold_italic face；Base14 字体会映射到对应的 Oblique / Italic face。`underline=True` 会按 span 绘制下划线，并在富文本换行和分页切片后保留。
